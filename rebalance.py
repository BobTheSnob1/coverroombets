#!/usr/bin/env python3
"""
Rebalance a mock‐stock portfolio so that each owned position has the same total value.
...
Usage:
    python rebalance.py --portfolio portfolio.txt --market market.txt

Outputs a series of commands of the form:
    !buy TICKER QTY
    !sell TICKER QTY
which will rebalance your holdings so that each stock you own ends up with
(as close as possible) the same total value. Any leftover change that cannot be
spent on a whole share is minimized.
"""
...
def rebalance(cash, stocks, prices):
    ...
    Calculate trades to rebalance so each owned ticker ends up
    with (as close as possible) the same total value.  Return a list
    of strings like “!buy TICKER QTY” or “!sell TICKER QTY”.
    """

    # 1) Identify which tickers we actually own and grab their prices
    owned_tickers = sorted(stocks.keys())
    N = len(owned_tickers)

    # 2) Compute current value in each position
    current_value = {}
    for t in owned_tickers:
        if t not in prices:
            raise ValueError(f"Price for ticker '{t}' not found in market data.")
        current_value[t] = stocks[t] * prices[t]

    total_stock_value = sum(current_value.values())
    total_portfolio_value = total_stock_value + cash

    # 3) The “ideal” per-stock value if we split total_portfolio_value evenly
    #    among the N tickers we own.
    ...
    cost_floor = sum(new_shares[t] * prices[t] for t in owned_tickers)
    ...
    # 7) Build trade commands based on the difference between new_shares and stocks[t]
    commands = []
    for t in owned_tickers:
        diff = new_shares[t] - stocks[t]
        if diff > 0:
...
def main():
    parser = argparse.ArgumentParser(
        description="Rebalance a mock-stock portfolio so that each owned position has equal value."
    )
    parser.add_argument(
        "--portfolio",
        "-p",
